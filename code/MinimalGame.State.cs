...
partial class MinimalGame : Game
{
    ...
    [Net] public GameStates GameState { get; set; } = GameStates.Live;
    ...
    {
        All.OfType<Glizzy>().ToList().ForEach(x => { _ = x.DeleteGlizzy(1); });
        All.OfType<MinimalPlayer>().ToList().ForEach(x => { x.CameraMode = new SpectateRagdollCamera(); });
    }
    ...
    private void FreshStart()
    {
        ...
        All.OfType<MinimalPlayer>().ToList().ForEach(x => { x.Respawn(); });
    }
